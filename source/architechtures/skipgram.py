...
def run() -> None:
    ...
    model = models.ModelSkipGram(config.device, config.vocabulary_size, config.embedding_size, vocabulary.padding_index)
    model.fit(
        training_dataloader,
    ...
    utils.plot_embeddings(embeddings, data_directory="skipgram")
